# Graph Report - BIS_AGENT  (2026-09-16)

## Corpus Check
- 59 files · ~62,786 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 10 file(s) not represented in the graph (top: (none) 5, .css 2, .example 1)

## Summary
- 733 nodes · 1277 edges · 47 communities (41 shown, 6 thin omitted)
...
- tools/__init__.py
- nodes.py
- change_user_password_in_supabase
- _BM25Index
- schema.sql
- process_query
- config.py
...
- `process_query()` --uses--> `SessionRateLimitException`  [INFERRED]
  app.py → src/agent/session.py
- `process_query()` --uses--> `APIErrorPayload`  [INFERRED]
  app.py → src/schemas.py
...
### Community 1 - "App.tsx"
Cohesion: 0.08
Nodes (48): App(), AuditLedgerModal(), AuditLedgerModalProps, AuthModal(), AuthModalProps, BisLogo(), BisLogoProps, ChatWindow() (+40 more)

### Community 2 - "AgentNodes"
Cohesion: 0.12
Nodes (19): AgentNodes, _get_scraped_data(), _is_difference_query(), _is_research_query(), Detect if the user is requesting a comprehensive compliance research…, Detect if the user is asking to compare or find differences between standards,…, Encapsulates graph node methods and initialized LLM/retriever dependencies., Rewrite a context-dependent query into a standalone query. Args:… (+11 more)

### Community 3 - "app.py"
Cohesion: 0.11
Nodes (32): AuthResponse, CacheClearResponse, clear_cache(), export_compliance_pdf(), get_keep_alive_status(), KeepAliveStatusResponse, list_active_sessions(), PDFExportRequest (+24 more)

### Community 4 - "supabase_client.py"
Cohesion: 0.11
Nodes (30): Supabase Database Integration Package., authenticate_user_in_supabase(), clear_all_audit_logs_from_supabase(), fetch_audit_logs_from_supabase(), get_async_db_engine(), get_session_from_supabase(), get_supabase_client(), get_user_account_usage_from_supabase() (+22 more)
...
### Community 6 - "Intent"
Cohesion: 0.33
Nodes (5): Enum, _node(), Factory: returns a retrieval node for domain PDFs using hybrid BM25 + dense…, Intent, str
...
Cohesion: 0.14
Nodes (21): format_markdown_tables(), guarantee_valid_pydantic_output(), Deterministic Output Guardrails & Post-Synthesis Validation Layer…, Ensure collapsed Markdown tables and headers have proper newline delimiters., Run output through strict Pydantic model validation. Guarantees fields like…, Validate, sanitize, and score a synthesized BIS response., validate_and_sanitize_response(), _needs_rewrite() (+13 more)

### Community 16 - "change_user_password_in_supabase"
Cohesion: 0.29
Nodes (7): change_password(), ChangePasswordRequest, Verify old password and update user password in Supabase PostgreSQL., change_user_password_in_supabase(), hash_password(), Hash password securely using SHA-256 with salt., Verify old password and update to new password in Supabase PostgreSQL.
...
Cohesion: 0.15
Nodes (12): process_query(), process_query_stream(), Any, QueryRequest, QueryResponse, Stream real-time model thinking, retrieval, and synthesis states as SSE events., Execute a turn of conversation with the BIS Agent. - Uses session history for…, get_user_model_config_from_supabase() (+4 more)

### Community 20 - "config.py"
Cohesion: 0.16
Nodes (14): display_server_banner(), main(), parse_args(), Namespace, BIS Agentic RAG Assistant - REST API Server Entry Point…, Parse CLI arguments for REST API server., Print styled startup banner for REST API., Validate environment and launch Uvicorn web server. (+6 more)
...
Cohesion: 0.24
Nodes (10): display_token_summary(), display_welcome_banner(), main(), parse_args(), Namespace, BIS Agentic RAG Assistant (Voice & Text Enabled)…, Parse command line arguments., Print styled startup banner with system configuration. (+2 more)

### Community 25 - "session.py"
Cohesion: 0.13
Nodes (8): build_graph(), LangGraph StateGraph Workflow Construction for BIS Agent., Construct and compile the LangGraph workflow., Agent orchestration package for BIS Agent., Conversational Session Management & Cache Bypass for BIS Agent., Tracks token usage across conversation turns and estimates savings from caching., TokenTracker, Response Cache ============== SHA-256 keyed cache for BIS responses with…
...
### Community 31 - "test_model_connection"
Cohesion: 0.22
Nodes (9): delete_session(), Instantiate target LLM model and verify connection with a test prompt., Clear conversation history and session memory for a given session_id from…, SimpleStatusResponse, test_model_connection(), TestModelConnectionRequest, delete_session_from_supabase(), Delete a session record from Supabase PostgreSQL. (+1 more)
...
### Community 35 - "process_voice_query"
Cohesion: 0.33
...
- **Why does `ResponseCache` connect `ResponseCache` to `AgentNodes`, `app.py`, `Session`, `tools/__init__.py`, `nodes.py`, `main.py`, `session.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `generate_compliance_pdf()` connect `generate_compliance_pdf` to `app.py`?**
