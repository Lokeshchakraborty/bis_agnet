...
class Config:
    ...
    history_window: int = int(os.getenv("BIS_HISTORY_WINDOW", "5"))
    max_stored_history: int = int(os.getenv("BIS_MAX_STORED_HISTORY", "8"))
    max_retries: int = int(os.getenv("BIS_LLM_MAX_RETRIES", "3"))
    retry_backoff_seconds: float = float(os.getenv("BIS_RETRY_BACKOFF", "1.5"))
    max_completion_tokens: int = int(os.getenv("BIS_MAX_COMPLETION_TOKENS", "2000"))
    context_max_chars: int = int(os.getenv("BIS_CONTEXT_MAX_CHARS", "1400"))
    scraper_timeout: float = float(os.getenv("BIS_SCRAPER_TIMEOUT", "2.5"))
    retrieval_timeout: float = float(os.getenv("BIS_RETRIEVAL_TIMEOUT", "3.0"))
